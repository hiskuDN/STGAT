...
with TimerBlock("Good Luck") as block:
    ...
    block.log('Model total number of params: ' +str(count_params(model)))
    ...
    if args.val_first:
        model.eval()
        loss, acc, score_dict, all_pre_true, wrong_path_pre_true, cls_acc  = val_net(data_loader_val, model, loss_function, global_step, args, val_writer)
    ...
    for epoch in process:
        ...
        global_step, mean_loss = train_net(data_loader_train, model, loss_function, optimizer, global_step, args, train_writer)
        block.log('       Mean training loss: {:.4f}                                                '.format(mean_loss))
        #block.log('Training finished for epoch {}'.format(epoch))
        model.eval()
        loss, acc, score_dict, all_pre_true, wrong_path_pre_true, cls_acc = val_net(data_loader_val, model, loss_function, global_step, args, val_writer)
        block.log('       Mean val loss: {:.4f}                                                     '.format(loss))
        block.log('       Test Acc: {:.4f}'.format(acc))
        ...
        block.log('Acc: {:4f}, Epoch_time: {:4f}, Best_acc: {:4f}\n'
                  .format(acc, time.time() - last_epoch_time, best_accu))
        ...
        if acc > best_accu:
            ...
            np.save(args.model_saved_name + '/cls_acc', cls_acc)
            torch.save(save, args.model_saved_name + '-best'+'-' + str(epoch) + '-' + str(global_step) + '.state')
