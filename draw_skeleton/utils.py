...
def write_skeleton(skeleton):
	'''
	To write the skeleton data into a file.
	'''
	
	# write it to a file called output.skeleton
	with open("output.skeleton", "w") as f:
		for frame in skeleton:
			f.write(frame + "\n")
			for subject in skeleton[frame]:
				f.write(subject + "\n")
				f.write(str(len(skeleton[frame][subject])) + "\n")
				for joint in skeleton[frame][subject]:
